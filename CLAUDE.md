...
- **Cross-module reads are allowed** (first used by Annual Plan's "Import from signup board…" picker, 2026-07-23): `GET /api/projects/:kind/:id` is scoped by `agency_id` + `kind` only, not by which module issued the request, so any module can read another kind's saved signups read-only via `useSignupList("<other kind>")` + a plain fetch — no backend change needed. Keep this one-directional (read another kind's payload to derive local state) — don't have one module write into another kind's `projects` row, and don't assume the shapes match without checking (Annual Plan's import works because a Resourcing payload's `board` array happens to already match `hoursByDowFromBoard`'s expected segment shape).
- **Outbound email** (added with self-service forgot-password, 2026-07-23) — the app's first and so far only email-sending code, `worker/email.js`, via Resend's REST API. Requires the `RESEND_API_KEY` secret (`wrangler secret put RESEND_API_KEY`, run by whoever owns the Resend account — never paste that key into a chat/tool transcript); unset, the feature silently no-ops server-side (logs why) rather than erroring the client. `RESET_EMAIL_FROM` (`wrangler.jsonc` vars) currently points at Resend's sandbox address, which can only deliver to the Resend account's own verified inbox — real delivery to arbitrary users needs a verified sending domain in Resend, a config change only. `/api/forgot-password` never reveals whether the submitted email matched an account (same generic response either way) — don't add a branch that would leak that.
