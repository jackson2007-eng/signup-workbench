...
- Sweep (in Auto-Build tab): builds a full board at each 10-hour target 0..max10 and ranks them. Exists because greedy is myopic — free choice ("value") can lose to forced structure ("fill tens first"), which a user correctly flagged as counterintuitive. The 10-hour count is the board's key structural variable; search it, don't hard-code it.
...
1. **Every weekly package costs 40 paid hours** (5×8 = 4×10), so "cost mode" at package level is meaningless — the real trade is coverage vs. four-day weeks operators value. That's why the mode is "fill the 10-hour allowance vs. let the score choose," swept numerically.
2. **Greedy anomalies are real**: constrained-first can beat free-choice. Answer with search breadth (sweep), not hand-tuned policies.
